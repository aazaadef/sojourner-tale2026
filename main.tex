...
\emph{Sojourner under Sabotage}~\cite{straubinger2025sojourner} is a browser-based serious game in which players repair sabotaged spaceship systems by writing JUnit tests and debugging mutated code across seven levels; an initial evaluation with 79~students showed over 80\% improvement in test coverage and debugging skills.
...
The pre-activity questionnaire collected background information (field, experience levels, prior tool use), 11~multiple-choice question (MCQ) items covering testing vs.\ debugging, unit/integration/black-box/white-box/mutation/functional testing and test case analysis, and four self-confidence Likert items (5-point scale). The post-activity questionnaire repeated the 11~MCQ and 4~self-confidence items and added perceived learning (4~items), engagement (3), usability (3), cognitive load (3) and four open-ended questions.
...
For those 21, a median of 40~minutes separated their first and last logged event, or 32~minutes once gaps longer than five minutes are excluded as idle time.

\begin{table}[htbp]
\caption{Telemetry event counts (22 students, 3,161 events)}
\label{tab:telemetry}
\centering
\small
...
Fifty-six \emph{ComponentDestroyedEvent}s indicate cases where students' tests failed to detect sabotage, triggering hidden tests as scaffolding (36~events).
...
For instructors considering the game: allow at least 70~minutes of play, since shorter sessions leave students in the tutorial level, and budget guidance for the first level, because orientation rather than syntax was what students most often found hard at the start.
